...
impl FlashAttn {
    ...
    >(
        ...
        if !(head_size_og == 256 || head_size_og == 128 || head_size_og == 64) {
            candle_core::bail!("only supports head dimension 64, 128 and 256 (got {head_size_og})")
        }
        if head_size_og % 8 != 0 {
...
impl candle_core::CustomOp3 for FlashAttn {
...
impl candle_core::CustomOp3 for FlashAttnVarLen {
    fn name(&self) -> &'static str {
        "flash-attn-varlen"
    }
